...
function isLikelyPlaceholder(v?: unknown) {
  if (!v) return false;
  const s = String(v).toLowerCase();
  return s.includes("yêu cầu sửa") || s.includes("require") && s.includes("edit") || s.includes("to be updated") || s.includes("tbd");
}

export default function ReviewerTopicSummary({
  topicDetail,
  submissionDetail,
  showAISection = true,
}: Props) {
  // merge topicVersion/currentVersion
  const mergeTopicData = (t?: any) => {
    if (!t) return null;
    const version = t.currentVersion ?? t.topicVersion ?? null;
    const base = t.topic ?? t;
    if (!version) return base;
    const out: Record<string, any> = { ...base };
    for (const k in version) {
      if (version[k] !== null && version[k] !== undefined) out[k] = version[k];
    }
    return out;
  };

  const displayedTopic = mergeTopicData(topicDetail) ?? undefined;

  const title =
    pickFirst(submissionDetail, ["topicTitle", "eN_Title", "vN_title"]) ??
    pickFirst(displayedTopic, ["eN_Title", "vN_title"]) ??
    `Submission #${submissionDetail?.id ?? "—"}`;

  const description =
    pickFirst(submissionDetail, ["additionalNotes", "description", "content"]) ??
    pickFirst(displayedTopic, ["description", "content"]) ??
    null;

  const tags = displayedTopic?.tags ?? submissionDetail?.tags ?? [];
  const keywords = displayedTopic?.keywords ?? displayedTopic?.keywordsList ?? null;

  const methodology =
    pickFirst(submissionDetail, ["methodology", "method", "approach"]) ??
    pickFirst(displayedTopic, ["methodology", "methodologyText", "approach"]);

  const expectedOutcomes =
    pickFirst(submissionDetail, [
      "expectedOutcomes",
      "expected_outcomes",
      "expectedOutcome",
      "expectedResults",
  ...
  const requirements =
    pickFirst(submissionDetail, ["requirements", "requirement", "requirementsText"]) ??
    pickFirst(displayedTopic, ["requirements", "requirement", "requirementsText"]);

  const aiRaw = submissionDetail?.aiCheckDetails ?? submissionDetail?.aiCheckDetailsRaw ?? null;
  ...
  const downloadAiPretty = () => {
    ...
    const pretty = (() => {
      try {
        return JSON.stringify(JSON.parse(aiCheck.raw), null, 2);
      } catch {
        return String(aiCheck.raw);
      }
    })();
  ...
  const submittedAt = submissionDetail?.submittedAt ?? submissionDetail?.submittedOn ?? null;
  const submissionStatus =
    submissionDetail?.status ??
    (displayedTopic ? (displayedTopic.latestSubmissionStatus ?? displayedTopic.status) : null);

  // Document
  const documentUrl =
    pickFirst(submissionDetail, ["documentUrl", "documentURL", "document"]) ??
    pickFirst(displayedTopic, ["documentUrl", "documentURL", "document"]) ??
    null;
  const fileId = pickFirst(submissionDetail, ["fileId"]) ?? pickFirst(displayedTopic, ["fileId"]) ?? null;

  const renderMaybePlaceholder = (label: string, value?: any) => {
    if (value == null || String(value).trim() === "") return null;
    const placeholder = isLikelyPlaceholder(value);
    return (
      <div className={`mt-3 text-sm ${placeholder ? "text-slate-500/80 italic" : "text-slate-600"}`}>
      ...
      <div className="bg-white border rounded-md p-4 w-full">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            {renderMaybePlaceholder("Phương pháp", methodology)}
            {renderMaybePlaceholder("Kết quả mong đợi", expectedOutcomes)}
            {renderMaybePlaceholder("Yêu cầu", requirements)}
          ...
          <div className="text-sm text-right min-w-[220px]">
            <div className="mb-2">
              <div className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${statusColorClass(submissionStatus ?? "")}`}>
                {submissionStatus ?? "—"}
              </div>
            </div>

            <div className="text-xs text-slate-500">Submitted</div>
            <div className="font-medium mb-2">{submittedAt ? formatDateTime(submittedAt) : "—"}</div>

            <div className="text-xs text-slate-500">Số lượng sinh viên tối đa</div>
            <div className="font-medium mb-2">{displayedTopic?.maxStudents ?? submissionDetail?.maxStudents ?? "—"}</div>
          </div>
        </div>

        {/* Document & Category / Semester */}
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-slate-700">
          <div>
            ...
            <div className="font-medium">{displayedTopic?.categoryName ?? submissionDetail?.categoryName ?? "—"}</div>
          </div>

          <div>
            <div className="text-xs text-slate-500">Kỳ học</div>
            <div className="font-medium">{displayedTopic?.semesterName ?? submissionDetail?.semesterName ?? "—"}</div>
          </div>
          ...
          <div>
            ...
            <div className="font-medium">
              {documentUrl ? (
                <a href={documentUrl} target="_blank" rel="noreferrer" className="text-sm text-blue-600 underline">Mở tài liệu</a>
              ) : fileId ? (
                <span className="text-sm text-slate-700">File #{fileId}</span>
              ) : (
                "—"
              )}
            </div>
        ...
        <div className="bg-white border rounded-md p-4 w-full overflow-x-auto">
          <div className="flex items-center justify-between">
            <div>
              ...
              {overallSummaryShort && <div className="mt-1 text-sm text-slate-500">{overallSummaryShort}</div>}
            </div>
