...
function getErrorDetail(err: unknown): string {
  ...
  return "Không kiểm tra được trùng lặp.";
}
...
export default function TopicEditFromSuggestionPage() {
  const { state } = useLocation() as { state?: NavState };
  const nav = useNavigate();
  const { data: categories = [] } = useCategories();
  const { data: semesters = [] } = useSemesters();

  const topicId = state?.formSnapshot?.topicId ?? 0;

  const [eN_Title, setENTitle] = useState(state?.formSnapshot?.eN_Title || "");
  const [vN_title, setVNTitle] = useState(state?.formSnapshot?.vN_title || "");
  const [abbreviation, setAbbreviation] = useState(
    state?.formSnapshot?.abbreviation || "",
  );
  const [problem, setProblem] = useState(state?.formSnapshot?.problem || "");
  const [context, setContext] = useState(state?.formSnapshot?.context || "");
  const [content, setContent] = useState(state?.formSnapshot?.content || "");
  const [description, setDescription] = useState(
    state?.formSnapshot?.description || "",
  );
  const [objectives, setObjectives] = useState(
    state?.formSnapshot?.objectives || "",
  );
  const [categoryId, setCategoryId] = useState<number>(
    state?.formSnapshot?.categoryId || 0,
  );
  const [semesterId, setSemesterId] = useState<number>(
    state?.formSnapshot?.semesterId || 0,
  );
  const [maxStudents, setMaxStudents] = useState<number>(
    state?.formSnapshot?.maxStudents || 1,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const d = await getTopicDetail(topicId);
        setDetail(d);
      } catch (e: unknown) {
        setDetailErr(
          e instanceof Error ? e.message : "Không tải được thông tin hệ thống",
        );
      }
    })();
  }, [topicId]);
  ...
  const validate = () => {
    ...
    if (!semesterId) e.semesterId = "Vui lòng chọn học kỳ";
  ...
  const handleCheckDuplicate = async () => {
    ...
    try {
      const result: AdvancedDuplicateResponse =
        await checkDuplicateAdvanced(body);

      toast.success("Hoàn tất kiểm tra", { id: tId });
      nav("/supervisors/ai-check-duplicate/advanced-result", {
        state: {
          result,
          formSnapshot: {
            eN_Title,
            ...
            topicId: topicId || state?.formSnapshot?.topicId,
            __fromSuggestion: true,
          },
        },
      });
    } catch (err: unknown) {
      toast.error(getErrorDetail(err), { id: tId });
    } finally {
      setIsChecking(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/70">
                Sử dụng gợi ý để điền nhanh thông tin đề tài
              </p>
            </div>
          </div>
          <div className="w-48">
        ...
        <div className="space-y-4 xl:col-span-2">
          ...
          >
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <Field label="Học kỳ" required error={errors.semesterId}>
                <select
                  value={semesterId || ""}
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                {detail?.currentVersion?.documentUrl ? (
                  <FileAttachment url={detail.currentVersion.documentUrl} />
                ) : detail?.documentUrl ? (
                  <FileAttachment url={detail.documentUrl} />
                ) : (
          ...
          <SectionCard title="Thông tin hệ thống">
            <div className="space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between rounded-xl border p-3">
                ...
                <span className="font-medium">
                  <Badge>{detail?.hasSubmitted ? "Đã nộp" : "Chưa nộp"}</Badge>
                </span>
              </div>
              <div className="flex items-center justify-between rounded-xl border p-3">
