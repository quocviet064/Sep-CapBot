...
export default function TopicVersionCreateSuggestPage() {
  ...
  const location = useLocation() as {
    state?: {
      seed?: Partial<VersionSeed>;
      formSnapshot?: Partial<VersionSeed>;
      submissionId?: number;
    };
  };

  const incomingSeed =
    location.state?.seed ?? location.state?.formSnapshot ?? {};
  ...
  useEffect(() => {
    if (topicError) toast.error("Không tải được dữ liệu đề tài để điền sẵn");
  }, [topicError]);
  ...
  const initialSeed: VersionSeed = {
    eN_Title: incomingSeed.eN_Title ?? fetchedSeed?.eN_Title ?? "",
    vN_title: incomingSeed.vN_title ?? fetchedSeed?.vN_title ?? "",
    description: incomingSeed.description ?? fetchedSeed?.description ?? "",
    objectives: incomingSeed.objectives ?? fetchedSeed?.objectives ?? "",
    methodology: incomingSeed.methodology ?? fetchedSeed?.methodology ?? "",
    expectedOutcomes:
      incomingSeed.expectedOutcomes ?? fetchedSeed?.expectedOutcomes ?? "",
    requirements: incomingSeed.requirements ?? fetchedSeed?.requirements ?? "",
    problem: incomingSeed.problem ?? fetchedSeed?.problem ?? "",
    context: incomingSeed.context ?? fetchedSeed?.context ?? "",
    content: incomingSeed.content ?? fetchedSeed?.content ?? "",
    supervisorId: incomingSeed.supervisorId ?? fetchedSeed?.supervisorId ?? 0,
    supervisorName:
      incomingSeed.supervisorName ?? fetchedSeed?.supervisorName ?? "",
    categoryId: incomingSeed.categoryId ?? fetchedSeed?.categoryId ?? 0,
  ...
  const [prevDocMeta, setPrevDocMeta] = useState<{
    ...
    initialSeed.docFileName
      ? { name: initialSeed.docFileName, size: initialSeed.docFileSize }
      : null,
  );
  ...
  if (loadingTopic) {
    return (
      <div className="rounded-xl border p-3 text-sm">Đang tải dữ liệu…</div>
    );
  }
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/70">Trạng thái: Draft</p>
            </div>
          </div>
          <div className="w-48">
                  ...
                  {!docFile && prevDocMeta && (
                    <div className="ml-auto flex max-h-20 max-w-[55%] flex-wrap items-center gap-2 overflow-y-auto">
                      <div
                        className="flex items-center gap-2 rounded-md border bg-white/80 px-2 py-1 text-[12px] shadow-sm"
                        onClick={(e) => e.stopPropagation()}
                        title="Tệp đã chọn trước đó - cần chọn lại để upload khi submit thật"
                      >
                        <FileText className="h-3.5 w-3.5" />
                        <span className="max-w-[160px] truncate">
                          {prevDocMeta.name}
