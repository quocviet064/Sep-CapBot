...
export default function TopicVersionCreateBackPage() {
  ...
  const saveDraft = (snap: FormSnapshot) => {
    try {
      ...
      return; // avoid no-empty: ignore storage errors
    }
  };
  const clearDraft = () => {
    try {
  ...
  const goCheckDuplicate = () => {
    ...
    const formSnapshot: FormSnapshot = {
      ...
      fileToken: seed.fileToken ?? null,
      docFileName: seed.docFileName ?? docFile?.name,
      docFileSize: seed.docFileSize ?? docFile?.size,
    };
        ...
        <div className="space-y-4 xl:col-span-2">
                    ...
                    <div className="ml-auto flex max-h-20 max-w-[55%] flex-wrap items-center gap-2 overflow-y-auto">
                      ...
                      >
                        ...
                        <span className="max-w-[160px] truncate">
                          {docFile?.name ?? seed.docFileName}
                        </span>
                        <span className="text-[10px] text-neutral-500">
                          {docFile
                            ? formatBytes(docFile.size)
                            : formatBytes(seed.docFileSize || 0)}
                        </span>
                        <button
                          type="button"
          ...
          <SectionCard title="Nội dung nghiên cứu" desc="Các trường bổ sung.">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              >
                <textarea
                  className="W-full min-h-[90px] rounded-xl border px-3 py-2 text-sm transition outline-none focus:border-neutral-800 focus:ring-2 focus:ring-neutral-900/10"
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="rounded-xl border p-3">
                ...
                <div className="text-sm font-medium">
                  {docFile || seed.docFileName ? `1 tệp` : "—"}
                </div>
                {(docFile || seed.docFileName) && (
